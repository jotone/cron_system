$(document).ready(function(){
	...
	$('button[name=save]').click(function(){
		...
		formData.append('also_reads', ($('input[name=also_reads]').prop('checked') == true)? 1: 0);
		$.ajax({
			url:		'/admin/news/add',
			type:		'POST',
			...
			success:function(data){
				try{
					...
					showErrors(e + data, '/admin/news/add')
				}
			}
		});
	});
	...
	$('.item-list a.drop').click(function(e){
		e.preventDefault();
		var result = confirm('Вы действительно хотите удалить новость '+$(this).attr('data-title')+'?');
		if(result){
			...
			$.ajax({
				...
				data:	{id:id},
				error:		function (jqXHR, textStatus, errorThrown) {
					showErrors(jqXHR.responseText, '/admin/news/drop')
				},
				success:function(data){
					try{
						data = JSON.parse(data);
						...
						showErrors(e + data, '/admin/news/drop')
					}
				}
			});
		}
