function sendPositions(){
	...
	$.ajax({
		...
		data:	{positions:positions, type:'top_menu'},
...
$(document).ready(function(){
	...
	$('button[name=save]').click(function(){
		...
		$.ajax({
			...
			data:		formData,
			error:		function (jqXHR, textStatus, errorThrown) {
				showErrors(jqXHR.responseText, '/admin/top_menu/add')
			},
			success:	function(data){
				try{
					data = JSON.parse(data);
					if (data['message'] == 'success') {
						location = '/admin/top_menu';
					}
				}catch(e){
					showErrors(e + data, '/admin/top_menu/add');
				}
			}
		})
	});
	...
	$('.categories-list-wrap a.drop').click(function(e){
		...
		if(result){
			...
			var _this = $(this);
			$.ajax({
				url:	'/admin/top_menu/drop',
				type:	'DELETE',
				headers:{'X-CSRF-TOKEN': $('header').attr('data-token')},
				data:	{id:id},
				error:		function (jqXHR, textStatus, errorThrown) {
					showErrors(jqXHR.responseText, '/admin/top_menu/drop')
				},
				success:function(data){
					try{
						data = JSON.parse(data);
						...
						showErrors(e + data, '/admin/top_menu/drop')
					}
				}
			});
		}
